...
## Vertical Modules and SaaS Tenancy

Industry-specific behavior must be implemented as versioned vertical modules inside the modular monolith. The generic worker, source adapters, repositories and scoring engine must not contain category-name conditionals.

Selecting `Anaokulu` resolves a `kindergarten` module that supplies provider-neutral search intents, qualification rules, enrichment policy and a versioned scoring profile. Source adapters remain provider-specific but industry-agnostic.

Kindergarten is the pilot, not a special case. Every future industry must be a separately versioned module using the same contracts and registry. Adding a sector may register a new module at the composition root, but must not require changes to the generic orchestrator, source adapters or existing sector modules.

Production data ownership is workspace-based. New repositories and jobs must receive a validated workspace context, preserve RLS isolation and attribute external API usage to that workspace. Billing-provider details must not leak into discovery modules.

Follow [MODULAR_SAAS_ARCHITECTURE.md](./MODULAR_SAAS_ARCHITECTURE.md) for contracts, boundaries and delivery slices. Do not begin its approval-gated implementation until the product owner approves the stated decisions.
